...
class DFComponentInstallerManager @Inject constructor(
    ...
    override suspend fun isComponentInstalled(componentName: String): Boolean {
        ...
        if (isInInstalledModules) {
            ...
            runCatching {
                ...
                Log.e(
                    TAG,
                    "isComponentInstalled($componentName): $isFullyInstalled (installedModules: $installedModules)"
                )
            ...
            Log.e(
                TAG,
                "Fallback to installedModules  session check fails $componentName so setting module installed to true"
            )
    ...
    override suspend fun installComponent(componentName: String): Flow<DFInstallProgress> { // Changed return type
        ...
        Log.d(TAG, "Build SplitInstallRequest for component: $componentName")
        ...
        try {
            ...
            return callbackFlow { // Flow<InstallProgress>
                ...
                Log.d(
                    TAG,
                    " callbackFlow --> Install requested for component: $componentName, currentSessionId: $currentSessionId"
                )
                val listener =
                    SplitInstallStateUpdatedListener { state -> // state is SplitInstallSessionState
                        Log.d(
                            TAG,
                            "Listener received state for $componentName: $state (Raw Status: ${state.status()})"
                        )
                        if (state.sessionId() == currentSessionId) {
                            // Map the Play Core state to our framework state AND create InstallProgress
                            val installProgress =
                                mapSessionStateToInstallProgress(state, componentName)
                            val newState =
                                installProgress.frameworkState // Extract framework state for logging/logic

                            Log.e(
                                TAG,
                                "Listener received state for $componentName (Session $currentSessionId): $newState (Raw Status: ${state.status()})"
                            )
                            ...
                            Log.e(
                                TAG,
                                "Listener ignored state update for unrelated session ${state.sessionId()}"
                            )
                        }
                    }

                try {
                    Log.d(TAG, "callbackFlow: Attempting to start install for '$componentName'")
                    currentSessionId = splitInstallManager.startInstall(request).await()
                    Log.e(
                        TAG,
                        "Install request initiated for $componentName. Session ID: $currentSessionId"
                    )
                    val existingListener = activeListeners.putIfAbsent(currentSessionId, listener)
                    Log.d(TAG, "callbackFlow: putIfAbsent result for session $currentSessionId. Existing? ${existingListener != null}")


                    if (existingListener == null) {
                        Log.d(TAG, "callbackFlow: Attempting to register listener for session $currentSessionId")
                        splitInstallManager.registerListener(listener)
                        Log.e(
                            TAG,
                            "Registered listener for $componentName (Session $currentSessionId)."
                        )
                        ...
                        Log.e(
                            TAG,
                            "Emitting initial state for $componentName (Session $currentSessionId): ${initialProgress.frameworkState}"
                        )
                        trySend(initialProgress)
                    } else {
                        Log.w(
                            TAG,
                            "Listener already active for session $currentSessionId. New listener not registered."
                        )
                        val currentSessionState =
                            splitInstallManager.getSessionState(currentSessionId).await()
                    ...
                    Log.e(
                        TAG,
                        "Failed to start install request or register listener for $componentName",
                        e
                    )
                    channel.trySend(DFInstallProgress(DFInstallationState.Failed(DFErrorCode.UNKNOWN_ERROR)))
                    channel.close(e) // Close the flow with the exception
                }

                awaitClose {
                    Log.e(
                        TAG,
                        "Flow closing (awaitClose) for $componentName (Session $currentSessionId). Ensuring listener cleanup."
                    )
                    cleanupListener(currentSessionId)
                }
            }
            ...
            Log.e(
                TAG,
                "Failed to create callbackFlow for $componentName: ${e.javaClass.name}, ${e.message}",
                e
            )
            return flowOf(DFInstallProgress(DFInstallationState.Failed(DFErrorCode.UNKNOWN_ERROR)))
        }
    }
    ...
    private fun cleanupListener(sessionId: Int) {
        ...
        if (listenerToRemove != null) {
            ...
            Log.w(
                TAG,
                "Attempted to clean up listener for session $sessionId, but it was not found in active map."
            )
        }
    }
    ...
    private fun mapSessionStateToInstallProgress(
        ...
        if (!state.moduleNames().contains(moduleName)) {
            Log.e(
                TAG,
                "State update for session ${state.sessionId()} does not list module $moduleName. Status: ${state.status()}"
            )
        ...
        val frameworkState: DFInstallationState = when (state.status()) {
            ...
            SplitInstallSessionStatus.FAILED -> DFInstallationState.Failed(
                DFErrorCode.fromSplitInstallErrorCode(
                    state.errorCode()
                )
            )
